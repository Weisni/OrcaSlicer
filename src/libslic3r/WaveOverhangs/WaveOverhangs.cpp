                                 ...
                                 coord_t                       connector_limit)
{
    ...
    auto append_or_start = [&](Polyline &&front) {
        ...
        if (d_flip < d_keep)
            front.reverse();
        const bool skip_first = current.last_point() == front.first_point();
        for (auto it = front.points.begin() + (skip_first ? 1 : 0); it != front.points.end(); ++it)
            current.polyline.append(Point3(*it, 0));
    };
