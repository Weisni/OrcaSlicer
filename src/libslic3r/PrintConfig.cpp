...
int DynamicPrintConfig::get_index_for_extruder(int extruder_or_filament_id, std::string id_name, ExtruderType extruder_type, NozzleVolumeType nozzle_volume_type, std::string variant_name, unsigned int stride) const
{
    ...
    if (variant_opt != nullptr) {
        ...
        // Hybrid nozzles do not have dedicated preset entries. Other specialized
        // flow variants may have one, so try their exact variant before falling
        // back to the standard values.
        if (nozzle_volume_type == NozzleVolumeType::nvtHybrid)
            nozzle_volume_type = NozzleVolumeType::nvtStandard;
